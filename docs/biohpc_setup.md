...
`singularity` module, GPUA100/GPUH100 partitions, `/project`–`/work`–`/archive`
filesystems). On another cluster the same steps apply — the site-specific
values all live in one config file, and anywhere marked `TODO` must be checked
against your own cluster's documentation or admins.
...
Two reasons: home quotas are small and the container image alone is ~10 GB, and
on some clusters home directories are not mounted on compute nodes at all.
...
| `APPTAINER_MODULE` / `APPTAINER_BIN` | `singularity` on UTSW BioHPC; `apptainer` on newer sites |
| `PROJECT_DIR` | this checkout, on `/project` or `/work` |
| `WSI_DIR` | where your slides live — this is what QuPath will browse |
| `BIND_PATHS` | filesystems to mount into the container (`/home2,/project,/work,/archive` on UTSW) |
...
## 3. Set up the container environment

```bash
bash slurm/build_container.sh
```

This needs **no build privileges** — deliberately, because regular cluster
users have no root and usually no `--fakeroot`. It does what BioHPC's own
Singularity guide recommends: pull a published Docker image and run it
directly.

1. Pulls the pinned MONAI base image (`projectmonai/monai:1.4.0` — CUDA,
   PyTorch and MONAI already matched to each other) into a single `.sif` file.
2. Creates a Python virtualenv **on shared storage, layered over the image**
   (`--system-site-packages`), and installs this project's extra dependencies
   into it — MONAI Label, shapely, and the `openslide-bin` wheel that carries
   the OpenSlide C library, so no system packages are ever needed.
3. Verifies every import, including `monailabel`, before declaring success.

The pull is ~10 GB and takes a while the first time; caches go to project
storage, not `/tmp`. If your site needs an outbound proxy for Docker Hub, set
`HTTP_PROXY`/`HTTPS_PROXY` in `config.env`.

(Sites that *do* allow unprivileged builds can bake everything into one image
instead with `bash slurm/build_container.sh --build-def`; then set `VENV_DIR=`
empty in `config.env`.)
...
Every line prints `PASS` or `FAIL`: module and binary, image and venv, bind
paths, **GPU visible inside the container with a supported compute
capability**, `monailabel` and OpenSlide imports, slide directory readable,
log directory writable. Do not continue until everything passes — every
confusing failure further down (silent CPU-only inference, jobs dying with no
output, container startup aborts) is one of these checks failing.
...
is a common source of breakage inside containers where the host driver is fixed
and not yours to change. Get things working first; treat cuCIM as a later
optimisation.
...
`build_container.sh` and `check_env.sh` verify the `monailabel` import so this
surfaces early.
...
```
$PROJECT_DIR/
├── monai_base.sif                the pulled container image
├── venv/                         project deps layered over the image
