...
A whole-slide pathology pipeline for UTSW BioHPC: annotate slides in QuPath, train from those annotations via a MONAI Label server on a GPU node, apply the model across a slide archive with SLURM job arrays. Everything runs on the cluster from a plain Python virtualenv on shared storage — no container runtime, no root.
...
**Setting up on BioHPC** (the task this repo exists for) — follow `docs/biohpc_setup.md` in order. The short form, run on a login node from `/project` or `/work` space (not `$HOME` — the venv is ~10 GB):

```bash
cp slurm/config.env.example slurm/config.env   # then edit: WSI_DIR + partition minimum
bash slurm/build_env.sh                        # plain venv + pip install; no root
...
- **No container layer, ever.** The container runtime was removed on request because it could not be deployed; the environment is one venv built by `slurm/build_env.sh` from `requirements.txt` + `requirements-train.txt`. Torch's PyPI wheels bundle CUDA 12 and target A100/H100; Pascal-era GPUp4 does not work with recent torch. A test in `tests/test_biohpc_layer.py` bans any reappearing container-runtime reference — do not reintroduce one.
...
Runtime environment: one virtualenv on shared storage (`VENV_DIR`), created by `slurm/build_env.sh` with plain `python -m venv` + pip. It holds the whole stack — torch with bundled CUDA from PyPI, MONAI, MONAI Label, and the `openslide-bin` wheel that supplies libopenslide without apt. All jobs run `$VENV_DIR/bin/...` directly; no module loads at job time.
